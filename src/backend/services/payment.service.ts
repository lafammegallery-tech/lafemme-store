  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!success) {
      return tx.payment.update({
        where: { id: payment.id },
        data: { status: "FAILED", failureReason: "Mock gateway rejected payment" },
      });
    }

    const updated = await tx.payment.update({
      where: { id: payment.id },
      data: { status: "SUCCESS", transactionId: `TRX-${Date.now()}`, paidAt: new Date() },
    });
      ...
      data: {
        ...
        statusHistory: {
          create: {
            ...
            toStatus: "CONFIRMED",
            note: "پرداخت آزمایشی با موفقیت تأیید شد.",
          },
        },
      },
    });
        ...
        ? [
            {
              ...
              type: "PAYMENT",
              title: "پرداخت موفق",
              message: `پرداخت سفارش ${payment.order.orderNumber} تأیید شد.`,
            },
          ]
        : [],
    });
